...
from ..tools.market_data_query import resample
...
def test_query_market_data_level_for_every_ticker(ticker):
    result = run_tool("query_market_data", {"ticker": ticker, "metric": "level", "granularity": "month"})
...
def test_query_market_data_weekly_return_pairs_with_bar_chart():
    result = run_tool(
        "query_market_data",
        {"ticker": "Nasdaq Composite", "metric": "return_pct", "granularity": "week", "start_date": "2026-01-01", "end_date": "2026-08-22"},
    )
