import test from "node:test";
import assert from "node:assert/strict";
import { seedAll } from "../../data/seed.js";
import { runConversation } from "./openai.js";

test.before(() => {
  seedAll();
});

const BAD_CHART = {
  chart_type: "area",
  title: "TVL",
  series: ["Aave", "Lido"], // area + 2 series -> invalid
  data: [{ x: "2026-01-01", Aave: 1, Lido: 2 }],
};

const GOOD_CHART = {
  chart_type: "line",
  title: "TVL",
  series: ["Aave", "Lido"],
  data: [{ x: "2026-01-01", Aave: 1, Lido: 2 }],
};

function toolCall(id, args) {
  return { id, type: "function", function: { name: "render_chart", arguments: JSON.stringify(args) } };
}

function assistantMessage(toolCalls) {
  return { choices: [{ message: { role: "assistant", content: null, tool_calls: toolCalls } }] };
}

test("an invalid render_chart call is corrected within the same turn, invisibly", async () => {
  let call = 0;
  const capturedMessages = [];
  const fakeClient = {
    chat: {
      completions: {
        create: async ({ messages }) => {
          call++;
          capturedMessages.push(structuredClone(messages));
          if (call === 1) return assistantMessage([toolCall("call_1", BAD_CHART)]);
          return assistantMessage([toolCall("call_2", GOOD_CHART)]);
        },
      },
    },
  };

  const result = await runConversation([{ role: "user", content: "chart TVL" }], "fake-key", fakeClient);

  assert.equal(call, 2, "should have retried exactly once after the bad chart");
  assert.deepEqual(result.chart, GOOD_CHART);

  const secondRequestMessages = capturedMessages[1];
  const toolResultMsg = secondRequestMessages.find((m) => m.role === "tool" && m.tool_call_id === "call_1");
  assert.match(toolResultMsg.content, /won't render well/i);
  assert.match(toolResultMsg.content, /muddy band/i);
});

test("gives up after 2 corrections and accepts the last attempt rather than returning nothing", async () => {
  let call = 0;
  const fakeClient = {
    chat: {
      completions: {
        create: async () => {
          call++;
          return assistantMessage([toolCall(`call_${call}`, BAD_CHART)]);
        },
      },
    },
  };

  const result = await runConversation([{ role: "user", content: "chart TVL" }], "fake-key", fakeClient);

  assert.equal(call, 3);
  assert.deepEqual(result.chart, BAD_CHART);
});

test("a valid chart on the first try is accepted with no retry", async () => {
  let call = 0;
  const fakeClient = {
    chat: {
      completions: {
        create: async () => {
          call++;
          return assistantMessage([toolCall("call_1", GOOD_CHART)]);
        },
      },
    },
  };

  const result = await runConversation([{ role: "user", content: "chart TVL" }], "fake-key", fakeClient);

  assert.equal(call, 1);
  assert.deepEqual(result.chart, GOOD_CHART);
});
